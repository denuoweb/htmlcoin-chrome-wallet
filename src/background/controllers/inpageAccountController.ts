...
import { MESSAGE_TYPE, PORT_NAME, HTMLCOINCHROME_ACCOUNT_CHANGE } from '../../constants';
...
export default class InpageAccountController extends IController {
  ...
  // Send message to and update htmlcoinchrome.account object of all registered ports
  public sendInpageAccountAllPorts = (statusChangeReason: HTMLCOINCHROME_ACCOUNT_CHANGE) => {
  ...
  // bg -> content script
  public sendInpageAccount = (port: any, statusChangeReason: HTMLCOINCHROME_ACCOUNT_CHANGE) => {
    port.postMessage({
      type: MESSAGE_TYPE.SEND_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES,
      accountWrapper: this.inpageAccountWrapper(statusChangeReason),
    });
  }

  private inpageAccountWrapper = (statusChangeReason: HTMLCOINCHROME_ACCOUNT_CHANGE) => {
  ...
  private handleLongLivedConnection = (port: any) => {
    ...
    */
    ...
    port.onMessage.addListener((msg: any) => {
      if (msg.type === MESSAGE_TYPE.GET_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES) {
        this.sendInpageAccount(port, HTMLCOINCHROME_ACCOUNT_CHANGE.DAPP_CONNECTION);
      }
    });
  }
