...
// Dapp developer triggers this event to set up window.htmlcoinchrome
...
async function setupLongLivedConnection(event: MessageEvent) {
  if (event.data.message && event.data.message.type === API_TYPE.CONNECT_INPAGE_HTMLCOINCHROME) {
    // Inject scripts
    ...
    port.onMessage.addListener((msg: any) => {
      if (msg.type === MESSAGE_TYPE.SEND_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES) {
        // content script -> inpage and/or Dapp event listener
        postWindowMessage(TARGET_NAME.INPAGE, {
          type: API_TYPE.SEND_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES,
    ...
    */
    ...
    postWindowMessage(TARGET_NAME.CONTENTSCRIPT, {
      type: API_TYPE.GET_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES,
      payload: {},
    });
  }
}
...
* event listeners and set window.htmlcoinchrome back to undefined, but there is no
* way to uninject the content scripts. This is not a big deal though as without a
* HtmlcoinChrome installation, the content scripts won't do anything (neither will the
* event listeners, but we can remove them so we may as well).
* And as long as the dapp implements the handleHtmlcoinChromeInstalledOrUpdated event
* listener, the page will be refreshed if HtmlcoinChrome is reinstalled.
*/
  ...
  chrome.runtime.sendMessage({ type: MESSAGE_TYPE.GET_LOGGED_IN_ACCOUNT }, (account: ICurrentAccount) => {
    if (!account) {
      ...
      postWindowMessage<IRPCCallResponse>(TARGET_NAME.INPAGE, {
        ...
        payload: {
          id,
          error: 'Not logged in. Please log in to HtmlcoinChrome first.',
        },
      });
      return;
    }
...
function forwardInpageAccountRequest() {
  port.postMessage({ type: MESSAGE_TYPE.GET_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES });
}
...
function handleInPageMessage(event: MessageEvent) {
  ...
  switch (message.type) {
    case API_TYPE.RPC_REQUEST:
      ...
      break;
    case API_TYPE.GET_INPAGE_HTMLCOINCHROME_ACCOUNT_VALUES:
